...
class TestUserEnumerationProtection:
    ...
    def test_record_login_attempt_success(self):
        ...
        should_block = protection.record_login_attempt(ip_address, username, success=True, user_exists=True)
    ...
    def test_record_login_attempt_failure_accumulation(self):
        ...
        for i in range(protection.max_attempts_per_ip - 1):
            should_block = protection.record_login_attempt(ip_address, username, success=False, user_exists=True)
            assert not should_block
    ...
    def test_ip_blocking_and_expiration(self):
        ...
        for _ in range(protection.max_attempts_per_ip):
            protection.record_login_attempt(ip_address, username, success=False, user_exists=True)
    ...
    def test_suspicious_pattern_detection(self):
        ...
        for username in usernames:
            protection._detect_suspicious_patterns(ip_address, username, user_exists=False, current_time=current_time)
    ...
    def test_is_ip_allowed_blocked_case(self):
        ...
        # Bloquear IP
        protection.record_login_attempt(ip_address, "user", success=False, user_exists=True)
...
class TestUserEnumerationProtectionGlobalFunctions:
    ...
    def test_record_login_attempt_global(self):
        ...
        should_block = record_login_attempt(ip_address, username, success=False, user_exists=True)
...
class TestUserEnumerationProtectionIntegration:
        ...
        for username in target_usernames:
            for attempt in range(2):
                blocked = protection.record_login_attempt(
                    attacker_ip, username, success=False, user_exists=(username in ["user1"])
                )
                if blocked:
                    break
            if blocked:
    ...
    def test_mixed_attack_legitimate_scenario(self):
        ...
        for i, username in enumerate(["admin", "root", "test", "user1", "guest"]):
            protection.record_login_attempt(attacker_ip, username, success=False, user_exists=False)
        
        # Usuario legítimo hace login exitoso
        protection.record_login_attempt(legitimate_ip, "john_doe", success=True, user_exists=True)
