-- consulta_optimizada_3.sql
-- Módulo: mantenimiento
-- Generado automáticamente el: 2025-08-22 23:18:58
-- Archivo original: D:\martin\Rexus.app\rexus\modules\mantenimiento\programacion_model.py
-- Línea original: 489
...
SELECT
    ...
    columna1,
    columna2,
    columna3 FROM plantillas_mantenimiento WHERE id = ? AND activo = 1

-- ÍNDICES SUGERIDOS PARA OPTIMIZACIÓN:
-- Ejecutar estos comandos para mejorar el rendimiento:

CREATE INDEX IF NOT EXISTS idx_mantenimiento_tabla_principal_id ON tabla_principal(id);
CREATE INDEX IF NOT EXISTS idx_mantenimiento_tabla_principal_activo ON tabla_principal(activo);
...
-- resultado = sql_manager.ejecutar_consulta_archivo('sql/mantenimiento/consulta_optimizada_3.sql', parametros)