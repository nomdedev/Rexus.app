...
# Script de validación de permisos iniciales para usuarios 'TEST_USER' y 'prueba' en SQL Server
...
WHERE u.usuario IN ('TEST_USER', 'prueba')
...
from dotenv import load_dotenv
        ...
        rows = cur.fetchall()
        if not rows:
            Logger().info("No se encontraron permisos para los usuarios 'TEST_USER' o 'prueba'.")
            conn.close()
            return
        Logger().info("Permisos de usuarios 'TEST_USER' y 'prueba':")
        for row in rows:
