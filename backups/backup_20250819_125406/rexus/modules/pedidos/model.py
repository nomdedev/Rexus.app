...
class PedidosModel:
    ...
    def __init__(self, db_connection=None):
        ...
        # Si no hay conexión, intentar establecer una automáticamente
        if not self.db_connection:
            try:
                from rexus.core.database import get_inventario_connection
                self.db_connection = get_inventario_connection()
                logger.info("[PEDIDOS] Conexión automática establecida exitosamente")
            except Exception as e:
                logger.warning(f"[PEDIDOS] No se pudo establecer conexión automática: {e} - usando modo demo")
        ...
        if not self.db_connection:
            ...
            logger.info("[PEDIDOS] Inicializado en modo completo con conexión BD")
...
None,
    ...
    def obtener_todos_pedidos(self) -> List[Dict[str, Any]]:
        """Alias conveniente para obtener todos los pedidos sin filtros."""
        return self.obtener_pedidos()

    def obtener_pedidos_base(self, filtros: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        """Obtiene pedidos usando la consulta SQL base directamente."""
        if not self.db_connection:
            return []
        ...
        try:
            ...
            # Usar la consulta base directamente
            query = self.sql_manager.get_query('pedidos', 'obtener_pedidos_base')
            ...
            logger.info(f"[PEDIDOS] Error en obtener_pedidos_base: {e}")
            return []
