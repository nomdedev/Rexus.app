...
def test_import(module_name, description=""):
    ...
    try:
        ...
        print(f"✅ {module_name} - Importado correctamente {description}")
        return True
    except ImportError as e:
        print(f"❌ {module_name} - Error de importación: {e} {description}")
        traceback.print_exc()
        return False
    except SyntaxError as e:
        print(f"❌ {module_name} - Error de sintaxis: {e} {description}")
        traceback.print_exc()
        return False
    except Exception as e:
        print(f"❌ {module_name} - Error inesperado: {e} {description}")
        traceback.print_exc()
        return False
...
def test_from_import(module_name, items, description=""):
    ...
    try:
        ...
        print(f"✅ {module_name} - Importaciones específicas correctas: {', '.join(items)} {description}")
        return True
    except ImportError as e:
        print(f"❌ {module_name} - Error de importación específica: {e} {description}")
        traceback.print_exc()
        return False
    except SyntaxError as e:
        print(f"❌ {module_name} - Error de sintaxis: {e} {description}")
...
def main():
    print("=" * 60)
    print("FASE 1: VERIFICACIÓN DE IMPORTACIÓN DE MÓDULOS CRÍTICOS")
    print("=" * 60)
    ...
    print("\n--- IMPORTACIONES ESPECÍFICAS CRÍTICAS ---")
    specific_imports = [
        ("rexus.utils.unified_sanitizer", ["sanitize_input", "validate_email", "sanitize_sql_query"]),
        ("rexus.utils.sql_script_loader", ["load_script", "execute_script", "get_script_path"]),
        ("rexus.core.auth_decorators", ["require_auth", "require_role", "log_access"]),
        ("rexus.utils.logging_config", ["setup_logging", "get_logger"]),
    ]
    ...
    if total_successful == total_tests:
        ...
        print("\n⚠️ HAY ERRORES DE IMPORTACIÓN QUE NECESITAN CORRECCIÓN")
        return False
