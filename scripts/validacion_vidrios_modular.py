...
def verificar_imports_submodulos():
    ...
    try:
        ...
        imports_exitosos += 1
        imprimir_resultado("ProductosManager importado", True, ["Gestión CRUD de vidrios"])
    ...
    try:
        ...
        imports_exitosos += 1
        imprimir_resultado("ObrasManager importado", True, ["Asignación de vidrios a obras"])
    except Exception as e:
    ...
    try:
        ...
        imports_exitosos += 1
        imprimir_resultado("ConsultasManager importado", True, ["Búsquedas y estadísticas"])
    except Exception as e:
    ...
    try:
        ...
        imports_exitosos += 1
        imprimir_resultado("ModeloVidriosRefactorizado importado", True, ["Orquestador modular"])
    except Exception as e:
...
def verificar_inicializacion_submodulos():
    ...
    managers_info = [
        ("ProductosManager", "rexus.modules.vidrios.submodules.productos_manager", "ProductosManager"),
        ("ObrasManager", "rexus.modules.vidrios.submodules.obras_manager", "ObrasManager"),
        ("ConsultasManager", "rexus.modules.vidrios.submodules.consultas_manager", "ConsultasManager")
    ]
...
def verificar_modelo_refactorizado():
    ...
    try:
        ...
        tiene_productos = hasattr(modelo, 'productos_manager') and modelo.productos_manager is not None
        tiene_obras = hasattr(modelo, 'obras_manager') and modelo.obras_manager is not None
        tiene_consultas = hasattr(modelo, 'consultas_manager') and modelo.consultas_manager is not None
        ...
        try:
            ...
            imprimir_resultado("Información modular", tiene_info, [f"Version: {info.get('version', 'N/A')}"])
        except Exception as e:
        ...
        try:
            ...
            modulos_conectados = sum(1 for v in conectividad.values() if v and v != False)  # Excluir db_connection
            ...
            imprimir_resultado(
                ...
                [f"{k}: {'✓' if v else '✗'}" for k, v in conectividad.items() if k != 'db_connection']
            )
        except Exception as e:
...
def verificar_compatibilidad_hacia_atras():
    ...
    try:
        ...
        try:
            ...
            imprimir_resultado("Alias ModeloVidrios", True, ["Para compatibilidad legacy"])
        except ImportError:
            imprimir_resultado("Alias ModeloVidrios", False)
        
        porcentaje_compatibilidad = (metodos_disponibles / len(metodos_compatibilidad)) * 100
