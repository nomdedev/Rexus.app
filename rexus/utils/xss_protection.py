...
class XSSProtection:
    ...
    def sanitize_html(html_content: str) -> str:
        ...
        for tag in allowed_tags:
            sanitized = re.sub(f'&lt;({tag})&gt;', r'<\1>', sanitized, flags=re.IGNORECASE)
            sanitized = re.sub(f'&lt;/({tag})&gt;', r'</\1>', sanitized, flags=re.IGNORECASE)
...
def sanitize_form_data(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key, value in data.items():
        if isinstance(value, str):
            ...
            clean_value = re.sub(r'<script[^>]*>.*?</script>', '', clean_value, flags=re.IGNORECASE | re.DOTALL)
            clean_value = re.sub(r'javascript\s*:', '', clean_value, flags=re.IGNORECASE)
            clean_value = re.sub(r'eval\s*\(', '', clean_value, flags=re.IGNORECASE)
