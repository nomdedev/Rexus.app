...
Proporciona sanitizadores especificos para diferentes tipos de datos
con enfoque en seguridad y prevencion de ataques.

Fecha: 15/08/2025
Componente: Seguridad - Sanitizacion de Datos
"""
...
class TextSanitizer:
    ...
    def sanitize_text(text: str, max_length: int = 1000, allow_html: bool = False) -> str:
        ...
        Args:
            ...
            allow_html: Si permite tags HTML basicos
    ...
    def sanitize_code(code: str) -> str:
        """Sanitiza codigos alfanumericos."""
    ...
    def sanitize_description(description: str) -> str:
        """Sanitiza descripciones permitiendo mas flexibilidad."""
...
class NumericSanitizer:
    """Sanitizador especializado para datos numericos."""
    ...
    def sanitize_integer(value: Any, min_value: Optional[int] = None,
        ...
        Returns:
            Optional[int]: Entero sanitizado o None si es invalido
        """
        try:
            ...
            if min_value is not None and int_value < min_value:
                logger.warning(f"Valor entero {int_value} menor que minimo {min_value}")
            if max_value is not None and int_value > max_value:
                logger.warning(f"Valor entero {int_value} mayor que maximo {max_value}")
            return int_value
        except (ValueError, TypeError):
            logger.error("No se pudo convertir valor a entero")
            return None

    @staticmethod
    def sanitize_decimal(value: Any, max_digits: int = 10,
                        decimal_places: int = 2) -> Optional[Decimal]:
        """Sanitiza y valida decimales."""
        try:
            if isinstance(value, str):
            ...
            # Verificar numero maximo de digitos
            if len(str(decimal_value).replace('.', '').replace('-', '')) > max_digits:
                logger.warning(f"Decimal {decimal_value} excede maximo de digitos {max_digits}")
            return decimal_value
        except (InvalidOperation, ValueError, TypeError):
            logger.error("No se pudo convertir valor a decimal")
            return None
...
class DateTimeSanitizer:
    ...
    def sanitize_date(value: Any, date_format: str = '%Y-%m-%d') -> Optional[date]:
        ...
        Returns:
            Optional[date]: Fecha sanitizada o None si es invalida
        """
        try:
            ...
            if isinstance(value, str):
                # Limpiar caracteres no validos para fechas
                value = re.sub(r'[^\d\-/:]', '', value)
                ...
                if min_date <= parsed_date <= max_date:
                    ...
                    logger.warning(f"Fecha {parsed_date} fuera de rango valido")
                    return None

        except (ValueError, TypeError):
            logger.error("No se pudo convertir valor a fecha")
            return None
        ...
        try:
            ...
            if isinstance(value, str):
                ...
                if min_datetime <= parsed_datetime <= max_datetime:
                    ...
                    logger.warning(f"DateTime {parsed_datetime} fuera de rango valido")
                    return None

        except (ValueError, TypeError):
            logger.error("No se pudo convertir valor a datetime")
            return None
...
class ContactSanitizer:
    ...
    def sanitize_email(email: str) -> Optional[str]:
        ...
        Returns:
            Optional[str]: Email sanitizado o None si es invalido
        """
        ...
        if not re.match(email_pattern, email):
            logger.warning(f"Email invalido: {email}")
            return None
        ...
        try:
            ...
            if len(local) > 64 or len(domain) > 253:
                logger.warning("Email excede longitud maxima permitida")
                return None
        except ValueError:
            return None
    ...
    def sanitize_phone(phone: str) -> Optional[str]:
        """Sanitiza numeros telefonicos."""
        if not isinstance(phone, str):
            return None

        # Remover todos los caracteres excepto digitos, +, -, (, ), espacios
        phone = re.sub(r'[^\d\+\-\(\)\s]', '', phone.strip())

        # Verificar longitud razonable
        digits_only = re.sub(r'[^\d]', '', phone)
        if len(digits_only) < 7 or len(digits_only) > 15:
            logger.warning(f"Telefono con longitud invalida: {phone}")
            return None
...
class SecuritySanitizer:
    """Sanitizador especializado para seguridad."""

    @staticmethod
    def sanitize_password(password: str) -> Optional[str]:
        """
        Sanitiza contrasenas (no las modifica, solo valida).

        Args:
            password: Contrasena a validar

        Returns:
            Optional[str]: Contrasena si es valida o None
        """
        ...
        if len(password) < 8:
            logger.warning("Contrasena demasiado corta")
            return None

        # Verificar longitud maxima (por seguridad)
        if len(password) > 128:
            logger.warning("Contrasena demasiado larga")
            return None

        # Verificar complejidad basica
        ...
        if not (has_upper and has_lower and has_digit):
            logger.warning("Contrasena no cumple complejidad minima")
            return None
    ...
    def hash_password(password: str) -> str:
        """Genera hash seguro de contrasena."""
    ...
    def verify_password(password: str, hashed: str) -> bool:
        """
        Verifica contrasena contra hash.
        ...
        Returns:
            bool: True si la contrasena es correcta
        """
        try:
...
class FileSanitizer:
    ...
    def sanitize_filename(filename: str) -> Optional[str]:
        ...
        Returns:
            Optional[str]: Nombre sanitizado o None si es invalido
        """
        ...
        if file_ext in dangerous_extensions:
            logger.warning(f"Extension de archivo peligrosa: {file_ext}")
            return None

        # Asegurar que no este vacio despues de sanitizacion
    ...
    def sanitize_file_path(file_path: str, base_directory: str) -> Optional[str]:
        """Sanitiza rutas de archivos previniendo path traversal."""
        ...
        try:
            ...
            # Verificar que la ruta este dentro del directorio base
            ...
            logger.error("Error al sanitizar ruta de archivo")
            return None


class SQLSanitizer:
    """Sanitizador especializado para identificadores SQL."""
    ...
    def sanitize_sql_identifier(identifier: str) -> Optional[str]:
        ...
        Returns:
            Optional[str]: Identificador sanitizado o None si es invalido
        """
        ...
        if not re.match(r'^[a-zA-Z_][a-zA-Z0-9_]*$', identifier):
            logger.warning(f"Identificador SQL invalido: {identifier}")
            return None
    ...
    def escape_sql_string(value: str) -> str:
        """
        Escapa strings para SQL (aunque se recomienda usar parametros).
        ...
        # Escapar comillas simples duplicandolas
...
class UnifiedSanitizer:
    ...
    def sanitize_by_type(self, value: Any, data_type: str, **kwargs) -> Any:
        """
        Sanitiza un valor segun su tipo.
