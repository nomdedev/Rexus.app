...
class PedidoDialog(QDialog):
    ...
    def get_datos_pedido(self):
        ...
        for row in range(self.tabla_productos.rowCount()):
            ...
            if all([codigo_widget, cantidad_widget, precio_widget, descuento_widget]):
                productos.append({
...
class PedidosViewComplete(BaseModuleView):
    ...
    def setup_panel_control(self, parent):
        ...
        self.cb_filtro_prioridad.addItems(["TODAS", "NORMAL", "ALTA", "URGENTE"])
    ...
    def llenar_tabla(self, pedidos):
        ...
        for row, pedido in enumerate(pedidos):
            self.tabla_pedidos.setItem(row, 0, QTableWidgetItem(pedido.get('codigo', '')))
            self.tabla_pedidos.setItem(row, 1, QTableWidgetItem(pedido.get('cliente', '')))
            self.tabla_pedidos.setItem(row, 2, QTableWidgetItem(pedido.get('obra', '')))
            self.tabla_pedidos.setItem(row, 3, QTableWidgetItem(pedido.get('fecha', '')))
            ...
            self.tabla_pedidos.setItem(row, 6, QTableWidgetItem(f"$ {pedido.get('total', 0):.2f}"))
            self.tabla_pedidos.setItem(row, 7, QTableWidgetItem(str(pedido.get('productos', 0))))
            self.tabla_pedidos.setItem(row, 8, QTableWidgetItem(pedido.get('observaciones', '')))
