...
class ObrasManager:
    ...
    def obtener_vidrios_por_obra(self, obra_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Sanitizar obra_id
            obra_id_sanitizado = self.data_sanitizer.sanitize_integer(obra_id, min_val=1)
            
            # Usar SQL externo para consulta
            query = self.sql_manager.get_query(self.sql_path, "obtener_vidrios_por_obra")
    ...
    def asignar_vidrio_obra(
        ...
        try:
            ...
            query_verificar = self.sql_manager.get_query(self.sql_path, "verificar_asignacion_existente")
            ...
            if cursor.fetchone():
                ...
                query = self.sql_manager.get_query(self.sql_path, "actualizar_asignacion_vidrio")
                cursor.execute(query, datos_asignacion)
            else:
                # Crear nueva asignación
                query = self.sql_manager.get_query(self.sql_path, "insertar_asignacion_vidrio")
                cursor.execute(query, datos_asignacion)
    ...
    def crear_pedido_obra(
        ...
        obra_id: int, 
        proveedor: str, 
        vidrios_lista: List[Dict[str, Any]]
    ) -> Optional[int]:
        ...
        try:
            ...
            for vidrio in vidrios_lista:
                cantidad = self.data_sanitizer.sanitize_integer(vidrio.get('cantidad', 0))
            ...
            query_pedido = self.sql_manager.get_query(self.sql_path, "crear_pedido_obra")
            cursor.execute(query_pedido, datos_pedido)
            
            # Obtener ID del pedido creado
            pedido_id = cursor.lastrowid or cursor.execute("SELECT @@IDENTITY").fetchone()[0]
            
            # Insertar detalles del pedido
            for vidrio in vidrios_lista:
                detalle = {
                    'pedido_id': pedido_id,
                    'vidrio_id': self.data_sanitizer.sanitize_integer(vidrio.get('vidrio_id', 0)),
                    'cantidad': self.data_sanitizer.sanitize_integer(vidrio.get('cantidad', 0)),
                    'precio_unitario': self.data_sanitizer.sanitize_numeric(vidrio.get('precio', 0)),
                    'subtotal': self.data_sanitizer.sanitize_integer(vidrio.get('cantidad', 0)) * 
                               self.data_sanitizer.sanitize_numeric(vidrio.get('precio', 0))
                }
                
                query_detalle = self.sql_manager.get_query(self.sql_path, "insertar_detalle_pedido")
    ...
    def actualizar_estado_pedido(self, pedido_id: int, nuevo_estado: str) -> bool:
        ...
        try:
            # Validar estado
            estados_validos = ['PENDIENTE', 'CONFIRMADO', 'EN_TRANSITO', 'ENTREGADO', 'CANCELADO']
            estado_sanitizado = self.data_sanitizer.sanitize_string(nuevo_estado).upper()
            ...
            query = self.sql_manager.get_query(self.sql_path, "actualizar_estado_pedido")
    ...
    def obtener_resumen_obra(self, obra_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            query_vidrios = self.sql_manager.get_query(self.sql_path, "contar_vidrios_obra")
            ...
            query_pedidos = self.sql_manager.get_query(self.sql_path, "resumen_pedidos_obra")
