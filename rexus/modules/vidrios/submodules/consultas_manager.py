...
class ConsultasManager:
    ...
    def obtener_todos_vidrios(
        ...
        try:
            ...
            if filtros:
                ...
                query = self.sql_manager.get_query(self.sql_path, "obtener_vidrios_filtrados")
                cursor.execute(query, filtros_sanitizados)
            else:
                query = self.sql_manager.get_query(self.sql_path, "obtener_todos_vidrios")
    ...
    def buscar_vidrios(self, termino_busqueda: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for row in cursor.fetchall():
                ...
                vidrio['relevancia'] = self._calcular_relevancia(vidrio, termino_sanitizado)
    ...
    def obtener_estadisticas_vidrios(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Total de vidrios
            query_total = self.sql_manager.get_query(self.sql_path, "contar_total_vidrios")
            cursor.execute(query_total)
            result = cursor.fetchone()
            estadisticas['total_vidrios'] = result[0] if result else 0

            # Valor total del inventario
            query_valor = self.sql_manager.get_query(self.sql_path, "calcular_valor_total")
            ...
            query_stock_bajo = self.sql_manager.get_query(self.sql_path, "vidrios_stock_bajo")
            cursor.execute(query_stock_bajo)
            result = cursor.fetchone()
            estadisticas['stock_bajo'] = result[0] if result else 0

            # Proveedores únicos
            query_proveedores = self.sql_manager.get_query(self.sql_path, "contar_proveedores")
    ...
    def obtener_vidrios_paginados(
        ...
        try:
            ...
            per_page = max(1, min(100, self.data_sanitizer.sanitize_integer(per_page, min_val=1)))
            ...
            if filtros:
                filtros_sanitizados = self._sanitizar_filtros(filtros)
                query_count = self.sql_manager.get_query(self.sql_path, "contar_vidrios_filtrados")
                cursor.execute(query_count, filtros_sanitizados)
            else:
                query_count = self.sql_manager.get_query(self.sql_path, "contar_total_vidrios")
            ...
            if filtros:
                ...
                query_data = self.sql_manager.get_query(self.sql_path, "obtener_vidrios_paginados_filtrados")
                cursor.execute(query_data, filtros_sanitizados)
            else:
                query_data = self.sql_manager.get_query(self.sql_path, "obtener_vidrios_paginados")
    ...
    def obtener_vidrios_stock_bajo(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            query = self.sql_manager.get_query(self.sql_path, "obtener_vidrios_stock_bajo")
            cursor.execute(query)
            ...
            for row in cursor.fetchall():
                ...
                vidrio['deficit_stock'] = vidrio.get('stock_minimo', 0) - vidrio.get('stock', 0)
                vidrios.append(vidrio)
    ...
    def _sanitizar_filtros(self, filtros: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for campo in campos_texto:
            if campo in filtros and filtros[campo]:
                filtros_sanitizados[campo] = self.data_sanitizer.sanitize_string(filtros[campo])
