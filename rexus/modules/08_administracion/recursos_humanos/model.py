...
h.empleado_id,
    ...
    def obtener_estadisticas_empleados(self):
        ...
        try:
            ...
            return {
                'total_empleados': int(row[0]) if row[0] is not None else 0,
                'nomina_mensual': float(row[1]) if row[1] is not None else 0.0,
                'por_estado': {
                    'ACTIVO': int(row[2]) if row[2] is not None else 0,
                    'INACTIVO': int(row[3]) if row[3] is not None else 0,
                    'VACACIONES': int(row[4]) if row[4] is not None else 0,
                    'LICENCIA': int(row[5]) if row[5] is not None else 0
                }
            }
