...
from PyQt6.QtWidgets import (
    ...
    QTabWidget, QScrollArea, QSplitter, QMessageBox, QCheckBox
)
...
class OrdenCompraDialog(QDialog):
    ...
    def calcular_totales(self):
        ...
        for row in range(self.tabla_productos.rowCount()):
            ...
            if all([cantidad_widget, precio_widget, descuento_widget, iva_widget]):
...
class ComprasViewComplete(BaseModuleView):
    ...
    def setup_panel_control(self, parent):
        ...
        self.cb_filtro_proveedor.addItems(["TODOS", "Proveedor ABC", "Suministros XYZ", "Materiales DEF"])
    ...
    def llenar_tabla(self, compras):
        ...
        for row, compra in enumerate(compras):
            self.tabla_compras.setItem(row, 0, QTableWidgetItem(compra.get('numero_orden', '')))
            self.tabla_compras.setItem(row, 1, QTableWidgetItem(compra.get('fecha', '')))
            self.tabla_compras.setItem(row, 2, QTableWidgetItem(compra.get('proveedor', '')))
            ...
            self.tabla_compras.setItem(row, 5, QTableWidgetItem(f"$ {compra.get('total', 0):.2f}"))
            self.tabla_compras.setItem(row, 6, QTableWidgetItem(str(compra.get('productos', 0))))
            self.tabla_compras.setItem(row, 7, QTableWidgetItem(compra.get('fecha_entrega', '')))
            self.tabla_compras.setItem(row, 8, QTableWidgetItem(compra.get('metodo_pago', '')))
            self.tabla_compras.setItem(row, 9, QTableWidgetItem(compra.get('observaciones', '')))
