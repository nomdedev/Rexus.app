...
class CategoriasManager:
    ...
    def renombrar_categoria(self, categoria_actual: str, categoria_nueva: str) -> Dict[str, Any]:
        ...
        try:
            ...
            cursor.execute(
                f"SELECT COUNT(*) FROM {self.TABLA_INVENTARIO} WHERE categoria = ?",
    ...
    def _existe_categoria(self, nombre_categoria: str) -> bool:
        ...
        try:
            ...
            # Verificar en tabla de inventario
            cursor.execute(
                f"SELECT COUNT(*) FROM {self.TABLA_INVENTARIO} WHERE categoria = ?",
                (nombre_categoria,)
            )
            existe_en_inventario = cursor.fetchone()[0] > 0
            
            # Verificar en tabla de categorías si existe
            if self._tabla_categorias_existe():
                cursor.execute(
                    f"SELECT COUNT(*) FROM {self.TABLA_CATEGORIAS} WHERE nombre = ? AND activa = 1",
                    (nombre_categoria,)
                )
            ...
            cursor.close()
            return existe_en_inventario or existe_en_categorias
