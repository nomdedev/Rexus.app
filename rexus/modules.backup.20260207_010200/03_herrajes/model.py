...
class HerrajesModel:
    ...
    def obtener_estadisticas(self) -> Dict:
        ...
        try:
            if self.sql_manager:
                # Usar SQL externo
                total_herrajes_result = self.sql_manager.execute_from_file('sql/03_herrajes/stats_total_herrajes.sql')
                total_herrajes = total_herrajes_result[0][''] if total_herrajes_result and len(total_herrajes_result) > 0 else 0

                total_stock_result = self.sql_manager.execute_from_file('sql/03_herrajes/stats_total_stock.sql')
                total_stock = total_stock_result[0][''] if total_stock_result and len(total_stock_result) > 0 else 0

                bajo_stock_result = self.sql_manager.execute_from_file('sql/03_herrajes/stats_herrajes_bajo_stock.sql')
                herrajes_bajo_stock = bajo_stock_result[0][''] if bajo_stock_result and len(bajo_stock_result) > 0 else 0

                proveedores_result = self.sql_manager.execute_from_file('sql/03_herrajes/stats_proveedores_activos.sql')
                proveedores_activos = proveedores_result[0][''] if proveedores_result and len(proveedores_result) > 0 else 0
            else:
                # Fallback con SQL directo
                cursor = self.db_connection.cursor()
