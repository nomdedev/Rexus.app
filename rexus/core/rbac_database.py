...
class RBACDatabase:
    ...
    def create_role(self, name: str, description: str, parent_id: Optional[int] = None) -> int:
        """
    ...
    def create_permission(self, resource: str, action: str, description: str = "") -> int:
        """
    ...
    def assign_permission_to_role(self, role_id: int, permission_id: int, granted_by: int) -> bool:
        """
    ...
    def check_access(self, user_id: int, resource: str, action: str, 
        ...
        try:
            ...
            policy_result = self._check_access_policies(user_id, resource, action, context)
            ...
            self._log_access_attempt(user_id, resource, action, result, context)
            ...
            log_security_event(
                "ACCESS_CHECK_ERROR",
                {"user_id": user_id, "resource": resource, "action": action, "error": str(e)},
                "ERROR"
            )
            
            return AccessResult.DENIED, "Error interno del sistema"
    
    def _check_user_permissions(self, user_id: int, resource: str, action: str) -> bool:
        ...
        try:
            ...
            with self._get_db_connection() as conn:
                ...
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?)
                """, (user_id, resource, action, result.value, json.dumps(details)))
