...
from rexus.core.auth_manager import UserRole, Permission
...
class UserManagementSystem:
    ...
    def validate_password(password: str) -> Tuple[bool, List[str]]:
        ...
        if len(password) < UserManagementSystem.MIN_PASSWORD_LENGTH:
            errors.append(f"La contraseña debe tener al menos {UserManagementSystem.MIN_PASSWORD_LENGTH} caracteres")
    ...
    @staticmethod
    def create_user(username: str, password: str, email: str, nombre: str, 
                   apellido: str, rol: str) -> Tuple[bool, str]:
    ...
    def update_user(username: str, data: Dict, current_user: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            if username.lower() == 'admin' and 'rol' in data and current_user.lower() != 'admin':
            ...
            if 'email' in data:
                valid_email, email_errors = UserManagementSystem.validate_email(data['email'])
                if not valid_email:
                    return False, f"Error en email: {', '.join(email_errors)}"
            ...
            if 'estado' in data:
                ...
                if data['estado'].lower() not in valid_states:
                    return False, f"Estado inválido. Estados válidos: {', '.join(valid_states)}"
    ...
    def change_password(username: str, old_password: str, new_password: str, 
        ...
        try:
            ...
            valid_pass, pass_errors = UserManagementSystem.validate_password(new_password)
            if not valid_pass:
