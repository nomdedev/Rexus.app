...
class PedidosModel:
    ...
    def create_tables(self):
        ...
        try:
            ...
            # Índices para optimización (SQL Server no soporta IF NOT EXISTS en CREATE INDEX)
            try:
                cursor.execute(
                ...
                if "already exists" not in str(e):
                    print(f"[PEDIDOS] Error creando índice idx_pedidos_numero: {e}")
            try:
                cursor.execute("CREATE INDEX idx_pedidos_estado ON pedidos(estado)")
            except Exception as e:
                if "already exists" not in str(e):
                    print(f"[PEDIDOS] Error creando índice idx_pedidos_estado: {e}")
            try:
                cursor.execute(
                ...
                if "already exists" not in str(e):
                    print(f"[PEDIDOS] Error creando índice idx_pedidos_fecha: {e}")
            try:
                cursor.execute("CREATE INDEX idx_pedidos_obra ON pedidos(obra_id)")
            except Exception as e:
                if "already exists" not in str(e):
                    print(f"[PEDIDOS] Error creando índice idx_pedidos_obra: {e}")
