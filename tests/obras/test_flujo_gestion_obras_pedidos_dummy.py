...
class DummyInventario:
    def __init__(self):
        self.stock = {"perfilA": 10, "vidrioA": 5, "herrajeA": 5}
...
class DummyVidrios:
    def reservar_vidrio(self, obra_id, tipo, cantidad):
        if cantidad > 5:
            raise ValueError("Stock insuficiente vidrio")
        return True
...
class DummyHerrajes:
    def reservar_herraje(self, obra_id, tipo, cantidad):
        if cantidad > 5:
            raise ValueError("Stock insuficiente herraje")
        return True
...
class DummyLogistica:
    def generar_envio(self, obra_id, datos_envio):
        if not datos_envio.get("direccion"):
