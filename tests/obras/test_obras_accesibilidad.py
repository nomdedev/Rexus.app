...
def test_obras_accesibilidad_tooltips(app):
    ...
    assert (
        boton.accessibleDescription() == "Botón principal para agregar una nueva obra"
    )
    # Botón verificar obra
    boton_verificar = view.boton_verificar_obra
    assert (
        boton_verificar.toolTip()
        == "Verificar existencia de obra en la base de datos SQL"
    )
    assert boton_verificar.accessibleName() == "Botón verificar obra en SQL"
    assert (
        boton_verificar.accessibleDescription()
        == "Botón para verificar si la obra existe en la base de datos SQL"
    )
    # Tabla principal
    ...
    assert (
        tabla.accessibleDescription() == "Tabla que muestra todas las obras registradas"
    )
    ...
    assert (
        view.label.accessibleDescription()
        == "Encabezado principal de la vista de obras"
    )
