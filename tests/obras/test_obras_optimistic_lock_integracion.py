...
def test_editar_obra_conflicto_rowversion(monkeypatch):
    ...
    try:
        ...
        if not hasattr(db, "connection") or db.connection is None:
            pytest.skip(
                "No hay conexión activa a la base de datos. Test de integración omitido."
            )
        with db.connection.cursor() as cursor:
            ...
            assert (
                row is not None
            ), "No se pudo insertar la obra de prueba. Verifica la base de datos y los permisos."
            id_obra = row[0]
        ...
        with db.connection.cursor() as cursor:
            ...
            assert (
                rowversion_nueva != rowversion_orig
            ), "La rowversion no cambió tras el update externo."
