    ...
    class LogisticaController:
        ...
        def intentar_colocacion(self, id_obra):
            if self.contab.esta_pagado(id_obra):
                ...
                self.view.mostrar_feedback(
                    "Colocación habilitada y registrada.", tipo="exito"
"""
            else:
                self.model.registrar_excepcion(
                    id_obra, self.usuario["usuario"], "Colocación sin pago"
                )
                self.view.mostrar_feedback(
                    "No se puede habilitar colocación: pago pendiente.", tipo="error"
                )
