...
@pytest.mark.parametrize(
    "tipo, item_id, ajuste, usuario",
    [
        ("perfil", 1, 5, "TEST_USER"),  # Suma 5 al stock del perfil 1
        ("item", 2, -3, "TEST_USER"),  # Resta 3 al stock del item 2
        ("perfil", 1, -20, "TEST_USER"),  # Edge case: stock negativo en perfil
        ("item", 2, 0, "TEST_USER"),  # Edge case: ajuste cero en item
        (
            "perfil",
            1,
            2,
            "usuario_inexistente",
        ),  # Edge case: usuario no válido en perfil
    ],
)
def test_ajuste_stock_y_movimiento(tipo, item_id, ajuste, usuario):
    ...
    conn_str = f"DRIVER={{{driver}}};SERVER={server};DATABASE={database};UID={username};PWD={password};TrustServerCertificate=yes;Timeout={timeout};"
    try:
        conn = pyodbc.connect(conn_str)
    except pyodbc.InterfaceError as e:
        raise RuntimeError(
            f"Error de conexión a la base de datos: {e}. Verifica las variables de entorno y los datos de conexión."
        ) from e
    cursor = conn.cursor()
    ...
    if tipo == "perfil":
        ...
        cursor.execute(
            "IF NOT EXISTS (SELECT 1 FROM inventario_perfiles WHERE id = ?) INSERT INTO inventario_perfiles (id, stock_actual) VALUES (?, 10)",
            (item_id, item_id),
        )
        conn.commit()
        # Obtener stock actual antes
        cursor.execute(
            "SELECT stock_actual FROM inventario_perfiles WHERE id = ?", (item_id,)
        )
        row = cursor.fetchone()
        ...
        cursor.execute(
            "UPDATE inventario_perfiles SET stock_actual = ? WHERE id = ?",
            (nuevo_stock, item_id),
        )
        ...
        ), f"El stock final ({stock_despues}) no coincide con el esperado ({nuevo_stock})"
        # Verificar movimiento registrado
        cursor.execute(
            "SELECT TOP 1 cantidad, usuario FROM movimientos_stock WHERE id_perfil = ? ORDER BY id DESC",
            (item_id,),
        )
        mov = cursor.fetchone()
        assert (
            mov and float(mov[0]) == float(ajuste) and mov[1] == usuario
        ), "El movimiento no fue registrado correctamente"
    else:  # tipo == "item"
        cursor.execute(
            """
        ...
        cursor.execute(
            "IF NOT EXISTS (SELECT 1 FROM inventario_items WHERE id = ?) INSERT INTO inventario_items (id, stock_actual) VALUES (?, 10)",
            (item_id, item_id),
        )
        conn.commit()
        # Obtener stock actual antes
        cursor.execute(
            "SELECT stock_actual FROM inventario_items WHERE id = ?", (item_id,)
        )
        row = cursor.fetchone()
        stock_antes = row[0] if row else 0
        # Realizar ajuste
        ...
        # Verificar movimiento registrado
        cursor.execute(
            "SELECT TOP 1 cantidad, usuario FROM movimientos_stock_items WHERE id_item = ? ORDER BY id DESC",
            (item_id,),
        )
        mov = cursor.fetchone()
        assert (
