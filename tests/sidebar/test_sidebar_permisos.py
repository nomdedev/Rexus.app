...
def test_sidebar_muestra_modulos_permitidos_admin(app):
    ...
    extras_presentes = [
        w.text()
        for w in sidebar.findChildren(type(sidebar._sidebar_buttons[0]))
        if w.text() in extras
    ]
    for extra in extras:
    ...
    for btn in sidebar._sidebar_buttons:
        assert (
            not btn.icon().isNull()
        ), f"El botón '{btn.text()}' no tiene ícono asignado"
    ...
    for extra_nombre, extra_icon in extras:
        btns = [
            w
            for w in sidebar.findChildren(type(sidebar._sidebar_buttons[0]))
            if w.text() == extra_nombre
        ]
        assert btns, f"No se encontró el botón '{extra_nombre}'"
        for btn in btns:
            assert _icon_cache_key(btn.icon()) == _icon_cache_key(
                extra_icon
            ), f"El ícono de '{extra_nombre}' no es el esperado"
    # 9. No hay íconos duplicados
    all_icons = [
        btn.icon().cacheKey()
        for btn in sidebar.findChildren(type(sidebar._sidebar_buttons[0]))
    ]
...
def test_sidebar_muestra_modulos_permitidos_usuario(app):
    ...
    assert "Configuración" in [
        w.text() for w in sidebar.findChildren(type(sidebar._sidebar_buttons[0]))
    ]
    # No duplicados
    assert len(titulos_sidebar) == len(set(titulos_sidebar))
    # Accesibilidad
