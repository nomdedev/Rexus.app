...
def test_login_exitoso(login_controller, login_view):
    ...
    assert login_controller.usuario_autenticado["usuario"] == "TEST_USER"
...
def test_login_usuario_inactivo(login_controller, login_view, usuarios_model):
    ...
    usuarios_model.usuarios["TEST_USER"]["estado"] = "inactivo"
    login_view.usuario_input.setText("TEST_USER")
    login_view.password_input.setText("password")
    login_controller.login()
    assert login_controller.usuario_autenticado is None
    assert "inactivo" in login_view.label_error.text().lower()
    usuarios_model.usuarios["TEST_USER"]["estado"] = "activo"  # restaurar
...
def test_login_usuario_sin_permisos(login_controller, login_view, usuarios_model):
    ...
    assert login_controller.usuario_autenticado is not None
    assert (
        "permiso" in login_view.label_error.text().lower()
        or "acceso" in login_view.label_error.text().lower()
    )
...
def test_login_error_conexion(monkeypatch, login_controller, login_view):
    ...
    monkeypatch.setattr(
        login_controller.usuarios_model, "obtener_usuario_por_nombre", lambda u: 1 / 0
    )
    login_view.usuario_input.setText("TEST_USER")
    login_view.password_input.setText("password")
    login_controller.login()
    assert (
        "error" in login_view.label_error.text().lower()
        or "excepción" in login_view.label_error.text().lower()
    )
...
def test_accesibilidad_foco_visible(login_view):
    ...
    for widget in [
        login_view.usuario_input,
        login_view.password_input,
        login_view.boton_login,
    ]:
