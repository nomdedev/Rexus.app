...
def get_db_connection(base):
    ...
    if base not in bases_permitidas:
        raise ValueError(f"Base de datos no permitida: {base}")
...
def obtener_conexion_a_base(base_name):
    """
    Obtiene una conexión a una base de datos de forma segura.

    Args:
        base_name (str): El nombre de la base de datos a la cual conectarse.

    Returns:
        pyodbc.Connection: La conexión a la base de datos.

    Raises:
        ValueError: Si el nombre de la base de datos no es válido.
    """
    # Lista blanca de bases de datos permitidas
    bases_permitidas = ['inventario', 'users', 'auditoria']

    if base_name not in bases_permitidas:
        raise ValueError(f"Base de datos no permitida: {base_name}")

    driver = os.environ.get('DB_DRIVER', 'ODBC Driver 17 for SQL Server')
    connection_string = (
        f"DRIVER={{{driver}}};"
        f"SERVER=localhost;"
        f"DATABASE={base_name};"
...
def test_tablas_extra_y_faltantes():
    ...
    for base in bases:
        # Validar que la base esté en la lista blanca
        if base not in bases_permitidas:
            print(f"ADVERTENCIA: Base de datos '{base}' no está en la lista de bases permitidas. Omitiendo prueba.")
            continue

        # Usar la función correcta que parametriza adecuadamente
        conn = obtener_conexion_a_base(base)
        cursor = conn.cursor()
