...
class MockController:
    def __init__(self, permisos, modulo):
        ...
        self.usuarios_model.tiene_permiso = lambda u, m, a: self.permisos.get(
            m, {}
        ).get(a, False)
        self.view = MagicMock()
    ...
    def accion_modificar(self):
        ...
        if not self.usuarios_model.tiene_permiso(
            self.usuario_actual, self.modulo, "modificar"
        ):
            self.view.label.setText(
                "No tiene permiso para realizar la acción: modificar"
            )
            return False
        return True
...
class TestPermisosUI(unittest.TestCase):
    ...
    def test_controlador_rechaza_accion_sin_permiso(self):
        ...
        controller.view.label.setText.assert_called_with(
            "No tiene permiso para realizar la acción: modificar"
        )
